...
int controller_getPath(char* path)
{
	...
	if(path != NULL)
	{
		printf("\nSugerencia: utilice la extensión '.csv' o '.txt'\n");
...
int controller_loadFromText(char* path , LinkedList* pArrayListMovie)
{
	...
	if(path != NULL && pArrayListMovie != NULL)
	{
		...
		if(f == NULL)
		{
			printf("Ese archivo no existe. Verifique si lo escribió correctamente o si olvidó la extensión.\n\n");
			return todoOk;
		}
...
int controller_filter(LinkedList* pArrayListMovie)
{
	...
	if(pArrayListMovie != NULL)
	{
		...
		printf("Seleccione el género que quiera filtrar (1 - 4): ");
		...
		case 1:
		{
			...
			{
				printf("No hay películas del género Drama.\n\n");
		...
		case 2:
		{
			...
			{
				printf("No hay películas del género Comedia.\n\n");
		...
		case 3:
		{
			...
			{
				printf("No hay películas del género Accion.\n\n");
		...
		default:
		{
			...
			{
				printf("No hay películas del género Terror.\n\n");
...
int controller_saveFilter(char* path , LinkedList* pArrayFilterList)
{
	if(path != NULL && pArrayFilterList != NULL)
	{
		...
		if(f == NULL)
		{
			printf("\nSe creó un nuevo archivo llamado '%s' con esta lista.\n\n", path);
		}else
		{
			printf("\nSe cargó esta lista en el archivo llamado '%s'\n\n", path);
		}
