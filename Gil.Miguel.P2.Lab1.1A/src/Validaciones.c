...
int validarPath(char* path)
{
	...
	{
		while(path == NULL || (strlen(path)) < 1 || (strlen(path)) > 49
			  || path[i] == 42 || path[i] == 47 || path[i] == 63 || path[i] == 92
			  || path[i] == 58 || path[i] == 34 || path[i] == 60 || path[i] == 62 || path[i] == 124)
		{
			fflush(stdin);
			printf("\nNombre de archivo inválido. Debe ingresar entre 1-49 caracteres.\n");
